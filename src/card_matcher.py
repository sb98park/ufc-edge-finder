...
# Re-exported so existing imports keep working; see src/names.py.
from src.names import _normalize_name, fight_key  # noqa: F401
...
def _agrees_with_market(row: dict) -> bool:
    """
    Model and market both make this selection more likely than not.

    On a moneyline that means they name the same winner. It generalises to
    every two-way market unchanged -- both sides saying "this happens" is the
    same test whether the selection is a fighter, an Over, or a method.
    """
    m, b = row.get("model_prob"), row.get("book_fair_prob")
    try:
        return float(m) > 0.5 and float(b) > 0.5
...
def top_standout_props(
    ...
    """
    The card's clearest reads: fights where the model and the market agree,
    ranked by the blended probability.

    THIS USED TO RANK BY EDGE, AND THAT WAS THE WRONG COHORT.

    Sorting by model-minus-market can only ever return fights where the two
    disagree, and this model disagrees by liking the underdog. Measured over
    7 cards of logged picks:

        model disagrees with market   n=23   model 62.1%   market 36.1%   won 39.1%
        model agrees with market      n=61   model 67.7%   market 71.9%   won 85.2%

    The market is nearly exact on the first line and the model is inverted, so
    the old rule pointed the most prominent surface on the site at the one
    cohort where its own logs say it is wrong. Every card it produced was a
    long underdog, which is not a quirk of one slate but what the ranking is.

    RE-SORTING BY EV WOULD HAVE CHANGED NOTHING, which is worth recording
    because it was tried first. EV is computed on the blended probability, so
    at a price near fair it reduces to roughly 0.30 x (model - market) x
    decimal -- the same disagreement, scaled. Measured on a live card the two
    rankings correlate at 0.94 and share 10 of their top 10. The fix has to be
    which fights QUALIFY, not the order they come back in.

    WHAT THIS IS AND IS NOT. It is not a positive-EV finder; on this evidence
    there is no such thing here, and most of what it returns prices negative
    once the vig is charged. It is the five calls the model is most confident
    about among those the market also backs, with what each costs -- which is
    how the reader was already building slips.

    `min_edge` is accepted and ignored, kept so the call sites that pass it
    keep working; there is no edge threshold in this rule by design.
    """
    if edges_df.empty:
        return []
    rows = edges_df.to_dict("records")
    agreed = [r for r in rows if _agrees_with_market(r)]
    if not agreed:
        return []
    agreed.sort(key=lambda r: -(r.get("blended_prob")
                                if r.get("blended_prob") == r.get("blended_prob")
                                and r.get("blended_prob") is not None
                                else float(r.get("model_prob") or 0)))
    # ONE PER FIGHT. Two selections on the same bout are the same read twice,
    # and a list of five that is really three fights hides how thin the card
    # is. Keyed on the pair rather than the market so a moneyline and a total
    # on one bout cannot both appear.
    seen, picked = set(), []
    for r in agreed:
        key = r.get("fight_id") or f"{r.get('fighter')}|{r.get('opponent')}"
        if key in seen:
            continue
        seen.add(key)
        picked.append(r)
        if len(picked) >= n:
            break
    return _decorate_props(picked, fighters_df)


def top_disagreement_props(
    edges_df: pd.DataFrame, fighters_df: pd.DataFrame | None = None, n: int = 5, min_edge: float = 5.0
) -> list[dict]:
    """
    Where the model disagrees with the market, kept but demoted.

    This is the old headline list. It is retained because the disagreements
    are real information and deleting them would be hiding the model's own
    behaviour -- but it is published behind a disclosure carrying the 39.1%
    record above, rather than as the thing the page leads with.
    """
    if edges_df.empty:
        return []
    rows = [r for r in edges_df.to_dict("records")
            if not _agrees_with_market(r) and (r.get("edge_pct") or 0) >= min_edge]
    if not rows:
        return []
    rows.sort(key=lambda r: -(r.get("edge_pct") or 0))
    seen, picked = set(), []
    for r in rows:
        key = r.get("fight_id") or f"{r.get('fighter')}|{r.get('opponent')}"
        if key in seen:
...
def _decorate_props(records: list[dict], fighters_df: pd.DataFrame | None) -> list[dict]:
    """Shared post-processing for both published lists."""
    for r in records:
...
# NOTE: a SECOND top_standout_props lived here and shadowed the real one.
# Python binds the last definition in a module, so the edge-ranked version
# below silently won over the agreement-ranked one defined earlier -- the
# selection rewrite looked applied, imported cleanly, and changed nothing on
# the page. Removed rather than renamed; there is one definition now.
