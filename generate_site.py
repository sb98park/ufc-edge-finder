...
def build_health_alerts(cards_df, predictions_path: str = f"{DATA_DIR}/predictions_log.csv") -> list:
    ...
    try:
        ...
        oa = {}
    # THE DUPLICATE-IDENTITY CHECK DELIBERATELY DOES NOT RENDER HERE.
    #
    # It was added to this strip and had to come straight back out. Every
    # other alert in this function is TRANSIENT and self-clearing -- a step
    # recovers, odds refresh, a cancelled fight resolves -- so the strip is
    # empty on a healthy build and its presence means something. A duplicate
    # identity persists until a human merges it, so three findings sat
    # permanently above the fight card, and the detail line told the site's
    # readers to edit src/names.py.
    #
    # Findings go to the GitHub step summary instead, which is where a
    # standing maintenance item belongs. See
    # scripts/check_duplicate_identities.py.
